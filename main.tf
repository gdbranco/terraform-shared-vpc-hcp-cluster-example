...
#   provider = aws.network-owner
#   name     = "${aws_iam_role.route53_role.name}-restrict-policy"
#   policy = replace(
#     replace(
#       file("./assets/restrict-route53-perms.json"),
#     "%%{ingress_hosted_zone_arn}", aws_route53_zone.ingress_private_hosted_zone.arn),
#   "%%{hcp_internal_communication_hosted_zone_arn}", aws_route53_zone.hcp_internal_communication_hosted_zone.arn)
# }
...
#   count    = var.restrict_shared_vpc_roles ? 1 : 0
#   provider = aws.network-owner
#   name     = "${aws_iam_role.vpce_role.name}-restrict-policy"
#   policy   = replace(file("./assets/restrict-vpce-perms.json"), "%%{aws_vpc_id}", module.vpc.vpc_id)
# }
...
module "bastion_host" {
  ...
  source         = "github.com/terraform-redhat/terraform-rhcs-rosa-hcp/modules/bastion-host"
